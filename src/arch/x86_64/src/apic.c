...
timer_calibration_t calibrate_apic_timer(int hz)
{
    ...
    cpuWriteLapic(APIC_REGISTER_TIMER_DIV, 0b1011);    // divider set to 1.
    ...
    uint32_t tick_for_hz = (cpu_wait_tick * 100) / hz; // (tick_in_10_ms * 100hz)/expected hz
    uint32_t divider = find_best_divider(tick_for_hz);
    timer_calibration_t calibration = {.divider = divider, .tick_to_wait = tick_for_hz / divider};
