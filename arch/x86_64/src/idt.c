...
idt_entry create_idt_entry(uintptr_t handler, uint8_t ist, uint8_t idt_flags) {
  return (idt_entry){.attributes = idt_flags,
                     .ist = ist,
...
static idt_descriptor_t idt_descriptor = {
    .size = sizeof(idt_entry) * nIDT_ENTRY, .offset = (uintptr_t)(&idt)};
...
#define set_isr_entry(n)                                                       \
  idt.entries[n] =                                                             \
      create_idt_entry((uintptr_t)isr##n, 0, IDT_PRESENT | IDT_INTERRUPT)
...
void setup_idt(void) {
  ...
  asm volatile(
      "push rax; mov rax, cr4; or rax, (1<<10); mov cr4, rax; pop rax");
  ...
  LOG_OK("IDT loaded, IDT descriptor at {x}, IDT at {x} of size {d}",
         &idt_descriptor, idt_descriptor.offset, idt_descriptor.size);
}
...
void log_stackframe(volatile stackframe *regs) {
  LOG_INFO("Stackframe :\nr15 : {x}\tr14 : {x}\tr13 : {x}\tr12 : {x}\nr11 : "
           "{x}\tr10 : {x}\tr9 : {x}\tr8 : {x}\nrbp : {x}\t"
           "rdi : {x}\trsi : {x}\nrdx : {x}\trcx : {x}\trbx : {x}\nrax : "
           "{x}\trsp : {x}\tint_no : {x}\t err_code : {x}\nrip : {x}\tcs : "
           "{x}\trflags : {x}\nuseresp : {x} \tss : {x}",
           regs->r15, regs->r14, regs->r13, regs->r12, regs->r11, regs->r10,
           regs->r9, regs->r8, regs->rbp, regs->rdi, regs->rsi, regs->rdx,
           regs->rcx, regs->rbx, regs->rax, 0xdeadbeef, regs->int_no,
           regs->err_code, regs->rip, regs->cs, regs->rflags, regs->useresp,
           regs->ss);
}
...
void attach_isr(uint8_t interrupt_number,
                void (*isr_to_add)(volatile stackframe *)) {
  isr[interrupt_number] = isr_to_add;
  LOG_INFO("Attached isr at {x} to interrupt number {d}.", isr_to_add,
           interrupt_number);
}