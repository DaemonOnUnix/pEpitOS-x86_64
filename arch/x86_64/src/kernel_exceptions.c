...
void k_page_fault(volatile stackframe *regs) {
  ...
  LOG_PANIC("Page Fault due to: {s} | {s} | {s}",
            (regs->err_code & 0b0001 ? "Page protection violation"
                                     : "Not present page"),
            (regs->err_code & 0b0010 ? "Write access" : "Read Access"),
            (regs->err_code & 0b0100 ? "Fault in userspace"
                                     : "Fault in privileged segment"));
...
void change_debug_mode(volatile stackframe *regs) {
  LOG_ERR("Switching debug mode. Current is {s}",
          (regs->rflags & (1 << 8) ? "enabled" : "disabled"));
  regs->rflags ^= (1 << 8);
}
