...
uintptr_t get_frame() {
  ...
  if (!is_bitmap_set) {
    for (size_t i = prev_i; available[i].length; i++) {
      ...
      // LOG_INFO("align of {x} : {x}", available[i].base,
      // align(available[i].base));
      uint64_t current_frame = align(available[i].base) +
                               ((number_of_frames_without_bitmap_in_c)*pg_size);
      if (current_frame <
          align(available[i].base) + available[i].length - pg_size) {
        number_of_frames_without_bitmap_in_c++;
        number_of_frames_without_bitmap++;
        LOG_INFO("PMM : allocating frame without bitmap, number {d} at {x}.",
                 number_of_frames_without_bitmap, current_frame);
        // LOG_ERR("TODO : clean the allocated frames.");
        for (uint64_t *i = physical_to_stivale(current_frame);
             (uintptr_t)i <
             (uintptr_t)physical_to_stivale(current_frame + pg_size);
             i++)
          *i = 0;
        prev_i = i;
    ...
    size_t i = 0;
    while (i < nframes / 64 &&
           bitmap_addr[i] == 0xffffffffffffffff) //== (uint64_t)-1)
      i++;
    ...
    if (i != nframes / 64) {
      ...
      while (available[z].length) {
        uint64_t current_frame =
            align(available[z].base) + ((cur_frame_n)*pg_size);
        if (current_frame <
            align(available[z].base) + available[z].length - pg_size) {
...
void init_pmm(uintptr_t virtual_addr) {
  ...
  LOG_INFO("Number of frames previously allocated (without bitmap): {d}",
           number_of_frames_without_bitmap);

  for (uint64_t i = 0; i < number_of_frames_without_bitmap; i++)
    flip_frame(i);

  LOG_INFO("{d} frames set to used due to bitmap...",
           number_of_frames_without_bitmap);
  ...
  for (; available[i].length; i++) {
    ...
    loc_datas[i] = (localisation_data){.aligned_base = base,
                                       .first_frame_number = current_nframes,
                                       .length = available[i].length -
                                                 (base - available[i].base)};

    current_nframes += loc_datas[i].length / 0x1000;

    LOG_INFO("Chunk informations : Created entry {d} { base:= {x}, start frame "
             "index:= {x}, length := {x}",
             i, loc_datas[i].aligned_base, loc_datas[i].first_frame_number,
             loc_datas[i].length);
  }
...
void set_memory_map(struct stivale2_struct_tag_memmap *memmap_tag) {
  LOG_INFO("PMM : got the memory map at {x}, of size {d}", memmap_tag->memmap,
           memmap_tag->entries);
  ...
  for (size_t i = 0; i < entry_number; i++) {
    ...
    if (x.type & 1 && !(x.type ^ 1))
      LOG_INFO(
          "Entry {d} := Base : {x}, Length : {d}, Type : {x}, Unused : {d}", i,
          x.base, x.length, x.type, x.unused);
  }
...
void free_frame(uintptr_t address) {
  ...
  LOG_INFO("PMM : Asking to free address {x}, freeing frame at {x}", address,
           addr);

  size_t i = 0;
  while (loc_datas[i].length) {

    LOG_INFO("Chunk informations : Examining entry {d} { base:= {x}, start "
             "frame index:= {x}, length := {x}",
             i, loc_datas[i].aligned_base, loc_datas[i].first_frame_number,
             loc_datas[i].length);

    if (addr >= loc_datas[i].aligned_base + loc_datas[i].length ||
        addr < loc_datas[i].aligned_base) {
      i++;
      continue;
    }

    uint64_t frame_number = loc_datas[i].first_frame_number +
                            (addr - loc_datas[i].aligned_base) / pg_size;
    LOG_INFO("PMM : Freed frame number {d} matching address {x} in block "
             "number {d}.",
             frame_number, addr, i);
    // LOG_INFO("{d} + ({x} - {x}) / {d} = {d};",
    // loc_datas[i].first_frame_number, addr, loc_datas[i].aligned_base,
    // pg_size, frame_number);
