...
gdt_entry create_gdt_entry(uint32_t base, uint32_t limit, uint8_t granularity,
                           uint8_t flags) {
  return (gdt_entry){
...
#define quick_entry(flags, granularity)                                        \
  (create_gdt_entry(0, 0, granularity, flags))
...
void setup_gdt() {
  ...
  gdt.entries[GDT_KERNEL_CODE] =
      quick_entry(GDT_PRESENT | GDT_SEGMENT | GDT_READWRITE | GDT_EXECUTABLE,
                  GDT_LONG_MODE_GRANULARITY);
  LOG_INFO("Kernel code descriptor created, at offset {x}.",
           GDT_KERNEL_CODE * sizeof(gdt_entry));

  gdt.entries[GDT_KERNEL_DATA] =
      quick_entry(GDT_PRESENT | GDT_SEGMENT | GDT_READWRITE, 0);
  LOG_INFO("Kernel data descriptor created, at offset {x}.",
           GDT_KERNEL_DATA * sizeof(gdt_entry));

  gdt_d.offset = (uint64_t)(&gdt);
  gdt_d.size = sizeof(gdt_t) - 1;
  LOG_INFO("GDT Descriptor created, at {x}. GDT at {x} with size {d}.",
           (uint64_t)(&gdt_d), gdt_d.offset, gdt_d.size);
